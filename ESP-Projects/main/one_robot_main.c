...
void app_main()
{
    ...
    xTaskCreate((void *)(current_voltage_update_task), "current_voltage_update", 4096, &pdh, 7, &current_voltage_update_handle);
    xTaskCreate((void *)(position_update_task), "position_update", 4096, NULL, 7, &position_update_handle);
    xTaskCreate((void *)(UART_rx_task), "uart_rx", 4096, NULL, 9, &uart_rx_handle);
    xTaskCreate((void *)(UART_tx_task), "uart_tx", 4096, NULL, 7, &uart_tx_handle);
    xTaskCreate((void *)(CAN_enable_task), "can_enable", 4096, NULL, 10, &can_enable_handle);
